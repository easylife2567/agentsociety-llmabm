...
1. **别人已做的（各环节分别有高质量研究）**：名人死亡×社交媒体哀悼（李文亮 134 万条计算分析、Jade Goody 案例、袁隆平/金庸记忆书写）；推荐系统曝光机制（TikTok 木偶审计、YouTube 漂移、Twitter Level II 随机实验）；重复曝光→认知（illusory truth Level II 系列）；去语境化/视觉误导（N=1,404 实验）。
...
3. **机制链证据强度**（RQ5 分级）：内容截取**强**（H1/H2）；重复曝光**强**（H3/H4/H5/H6）；认知收敛**强但结果变量非人物印象**（H7/H8）→「曝光→印象单一化」链节为[推断]，**中等偏弱**；低熟悉首因印象**中**（H9）。
4. **两个必须诚实面对的前提**：
...
## 附录 A：机制链各环节代表论文（2026-08-19，核心证据集）

> 选取口径：79 条 core 中证据质量最高者，按机制链四环节 + 两条横切线归组；篇数不限。材料层级：A=全文精读卡片，B=CNKI 摘要级，C=题录级（未精读，仅作弱证据）。证据等级沿用卡片体系：Level II 随机实验＞III 受控审计＞IV 计算观察＞VI/VII 质性/概念。**诚实标注**：L046 效应具议题依赖性、L016 为观察设计、D 系列结果变量是命题真实性而非人物印象——凡此，均直接写入「链条中的角色」。

### A.1 环节一：内容截取与去语境化（证据强度：强）

| ID | 题名 | 期刊（年份） | 材料/等级 | 关键发现 | 链条中的角色 |
|---|---|---|---|---|---|
| C27 | 从「完整观看」到「切片判断」：短视频传播中的去语境化评价 | 榆林学院学报（2026） | B 摘要级 | 切片观看→评价集中化、稳定化 | 第一环节的本土直接证据 |
| L046 | A Picture Paints a Thousand Lies? The Effects of Reshared News | Political Communication（2020） | A 卡片 / II | N=1,404 图文重配实验：视觉去语境化可信度效应具**议题依赖性**（校园枪击无稳定差异） | 校准警示：效应非普适，须在张雪峰议题重测 |
...
| C28 | 算法偏见与乡土想象：短视频平台受众对乡村形象的认知形塑 | 新闻前哨（2025） | B 摘要级 | 算法×受众认知→乡村形象单一化、标签化 | 邻近机制类比：算法→形象刻板化 |

### A.2 环节二：少数片段重复推荐 / 曝光集中（证据强度：强）

| ID | 题名 | 期刊（年份） | 材料/等级 | 关键发现 | 链条中的角色 |
|---|---|---|---|---|---|
| L031 | An Empirical Investigation of Personalization Factors on TikTok | Proc. ACM Web Conf.（2022） | A 卡片 / III 木偶审计 | 因素隔离：关注＞点赞≈观看时长；交互→曝光正反馈 | 抖音同平台曝光机制（互动权重 w） |
| L040 | Algorithmic amplification of politics on Twitter | PNAS（2022） | A 卡片 / II 随机实验 | 近 200 万日活随机对照：7 国中 6 国放大主流政党 | 排序放大因果基准（放大比 A） |
...
| L022 | The bias beneath: analyzing drift in YouTube's recommender | Social Network Analysis and Mining（2024） | A 卡片 / IV | 四层推荐网络，Atkinson 不平等指数随深度趋近 1 | 深度漂移/少数赢家 |
...
### A.3 环节三：重复曝光→认知收敛（证据强度：强；但结果变量是命题真实性，非人物印象）
...
| L060 | The effects of repetition frequency on the illusory truth effect | Cognitive Research（2021） | A 卡片 / II | 对数形频次效应：第二次曝光增量最大、之后边际递减（d≈1.00/0.67） | log 形状参数 a |
| L061 | Perceived truth of statements and simulated social media engagement | Cognitive Research（2020） | A 卡片 / II | 逐字/语义一致重复↑真实感；**语义不一致重复↓真实感** | 框架竞争胜负（一致性阈值 θ） |
...
### A.5 横切：哀悼场域与死后内容供给（RQ1 支撑）

| ID | 题名 | 期刊（年份） | 材料/等级 | 关键发现 | 链条中的角色 |
|---|---|---|---|---|---|
| L016 | Online propagation of emotions: celebrity suicides | PLOS ONE（2025） | A 卡片 / IV 计算观察 | 四起名人自杀百万级推文：厌恶快、广、持久；愤怒/惊讶快而短 | 情绪→再分发权重表；**观察设计，无法区分用户选择与平台排序** |
...
| C7 | 「逝去的歌」：B 站纪念账号的数字哀悼和媒介记忆建构 | 新闻与写作（2023） | B 摘要级 | 纪念账号的情感从悲伤延展至鼓励 | 本土平台哀悼仪式 |
| C0 | 数字永生：赛博空间中的记忆留存与伦理思辨 | 新闻爱好者（2026） | B 摘要级 | 个性化推送→社交扩散→仪式消费的链式传播结构 | 死后内容分发链（理论描述，非实测） |
...
### A.7 平台治理与闭环（H10/H11 支撑，证据多为摘要级——本节是全链最弱环节）

| ID | 题名 | 期刊（年份） | 材料/等级 | 关键发现 | 链条中的角色 |
|---|---|---|---|---|---|
| C8 | 「故园荒芜」：「数字死亡」的记忆与遗忘 | 国际新闻界（2023） | B 摘要级 | 平台存储与遗忘的权力 | 治理可切断回路 |
| C15 | 新生活（快手悼念账号功能） | 小康（2021） | B 摘要级 | 悼念账号功能：平台主动参与死后内容 | 平台功能层 |
| C1 | 数字记忆视域下纪念账号的记忆建构与维系 | 全媒体探索（2025） | B 摘要级 | 纪念账号的记忆建构与维系 | 供给侧结构 |

### A.8 环节证据强度一览（RQ5 交叉结论）

```
内容截取 ──► 重复曝光 ──► 认知收敛 ──► 低熟悉首因印象
  强(C27/L047)   强(L031/L040)   强(L058/L060)   中(L052)
  校准警示(L046) 双层集中(L021/L022) 变量错位(L061→命题) 迁移须重测
  ◄──── H10 认知—供给闭环 中(L062/L016) ────►
  ◄── H11 哀悼/标签框架竞争 中(L011/C12/C2) 贯穿 ──►
```

- **最强链节**：重复曝光（A.2）与认知收敛（A.3）——Level II/III 证据齐备，可直接参数化；
- **变量错位**：A.3 全部实验的因变量是「命题真实性/可信度」，**「重复→人物整体印象单一化」无直接测量先例**（本 RQ 的 Novel 点）；
- **最弱链节**：A.7 平台治理对曝光回路的量化作用（摘要级/质性）；A.4 首因印象迁移须以张雪峰切片重测。
