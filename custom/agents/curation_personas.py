...
机制表现的类型差异锚定数据挖掘报告实证（activity 已按 2026-09-10 发帖人口径重锚，
见 _PARAM_SPECS 注释；旧内容口径的活跃度锚定随口径切换废弃）：
...
# CurationDiscourseAgent._speak_probability）。个体参数 = 均值 × U[0.8, 1.2] 抖动后截断。
#
# - activity: 基础发言倾向。用户 2026-09-10 裁定改按发帖人口径（玩梗18/悼念21/营销26/
#   教育15/其他20）后重锚：activity_mean = 0.60 × (该类型内容份额/作者份额)，
#   实证比值 meme 0.82 / mourning 0.89 / marketing 1.00 / education 1.02 / other 0.88
#   （W12-W22 注入池 48,360 帖），→ 0.49 / 0.53 / 0.60 / 0.61 / 0.53（各类型接近均匀，
#   类型分化主要由 spiral/decay/pressure 三机制承载）。
...
_PARAM_SPECS: dict[str, dict[str, tuple[float, float, float]]] = {
    ...
    "meme":      {"activity": (0.49, 0.2, 0.9), "spiral": (1.2, 0.3, 2.5),
                  "decay": (0.8, 0.2, 2.0),     "pressure": (0.9, 0.3, 1.0)},
    "mourning":  {"activity": (0.53, 0.2, 0.9), "spiral": (0.8, 0.2, 2.0),
                  "decay": (1.2, 0.4, 2.5),     "pressure": (-0.3, -0.6, 0.0)},
    "marketing": {"activity": (0.60, 0.3, 1.0), "spiral": (0.2, 0.0, 0.8),
                  "decay": (0.1, 0.0, 0.6),     "pressure": (0.5, 0.1, 1.0)},
    "education": {"activity": (0.61, 0.2, 1.0), "spiral": (0.5, 0.1, 1.5),
                  "decay": (0.4, 0.1, 1.2),     "pressure": (0.15, 0.0, 0.6)},
    "other":     {"activity": (0.53, 0.1, 0.8), "spiral": (1.5, 0.5, 3.0),
