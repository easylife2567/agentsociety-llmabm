...
2. **基于观察决定本周是否发言**。
   ...
   是否发言由数值决策算法决定：`U = B_i + R·(D−B_i)`。其中 B_i 是事件前常态表达
   效用锚点，D 是当前信息流意见气候形成的沉默螺旋因子，R 是重复曝光后的注意力余量；
   R 下降时效用回到常态锚点，而不是归零。U 与您的门槛 activity 比较，参数见您
   profile 中的 params；您只需按流程行动：若效用值未达门槛，保持沉默即可；
   若达到门槛，撰写一条符合您类型与人设的帖子。请注意：**每周至多发布 1 帖**。
