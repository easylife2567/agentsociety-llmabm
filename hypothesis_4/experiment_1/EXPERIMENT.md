...
**发言决策**（数值算法，无 LLM；**2026-09-14 用户裁定：锚定式表达效用**；
仍只有沉默螺旋与注意力衰减两条规则，原环境增益 G 维持退役）：

```
U_i = B_i + R_i·(D_i − B_i)                              （锚定式表达效用）
D_i = 1 + (alpha_D·s_i)·tanh(1.5·(share_own − base)/base)
R_i = exp(−λ_i·cum_own/15)
发言当且仅当 U ≥ activity
```

边界含义：`R=1 ⇒ U=D`，冲击期由当周意见气候决定；`R→0 ⇒ U→B_i`，注意力耗散后回到事件前常态盘而非完全沉默；`D=B_i ⇒ U=B_i`。`alpha_D` 乘在敏感度 `s_i` 上，不能直接乘整个 D，否则会错误移动 D=1 的中性中心。

| 类型 | 常态锚点 B_i | 标定后 λ_i 均值 |
|---|---:|---:|
| 玩梗 | 0.7755 | 1.243038 |
| 悼念 | 0.7678 | 2.282035 |
| 营销 | 0.82515 | 1.019241 |
...
全局 `alpha_D=0.4296954756`。`B_i` 由 W05–W12 常态供给映射到既有 activity 分布后固定；`alpha_D` 与五类 λ 只用 W13–W18 统一标定，W19–W22 为留出检验。λ 在每一类型内按原个体值同比缩放，保留既有异质性，不重新抽样。数值依据见 `data/anchored_utility_prediction.json`；该文件是100-seed数值代理，不是正式仿真结果。

R 的尺度（用户裁定沿革：50 → 20（2026-09-12）→ **15**（2026-09-13，"R 力度再大一些"），
λ 等效 ×3.33；一轮 50→20 的理由如下述，20→15 的复估见下段末）：真实平台总量是**深 V**
...
被动抬升；玩梗型累计曝光起步晚，二波不受损。**20 → 15 的复估（2026-09-13，30 seed 同构代理，
新群体 19/22/23/15/21 + W13 保底 + 官方置顶 + 营销 λ=0.05；**已含 2026-09-13 的同构缺口修正
与方案 B**）：W13 悼念份额 0.510 vs 真实 0.414（Δ +0.096，判据 ±0.08 ❌ —— 见下节 N 扫描）；
玩梗 W19–W22 0.002/0.228/0.501/0.610 vs 真实 0.078/0.298/0.556/0.603（二波形状仍贴合）；
五类构成平均 |差| 玩梗 0.029 / 教育 0.054 / 其他 0.077 / 悼念 0.079 /
**营销 0.214（最大 0.275 @W15，已知偏差）**。
（此前报的 W13 0.423–0.431 系校准器把 noise 并入 "other" 时的乐观值，已作废。）尺度定义于共享纯函数
...
- **候选池口径（用户 2026-09-13 裁定·方案 B）**：排名口径是"哀悼**倾向分**前 N"，不是
  ...
  缘由：倾向分 = 命中数/(字数/50)，短文本密度天然偏高，实测 2/3 个 seed 有 1 张 35–98 字的
  乱码/广告 noise 帖挤进全员强制位（构成为 mourning3+other1+noise1 / mourning4+marketing1 /
  mourning4+noise1）。否决"仅取 mourning 类型"档：同 N=5 下 W13 悼念份额 0.555 vs 本口径 0.515
  （Δ 分别 +0.141 / +0.101）——两档在 N=5 下**都**超判据，故该否决只说明"mourning 类型档更差"，
  并不说明本口径已达标；达标与否取决于 N（见下表）

#### ⚠ 保底条数 N 待裁定（2026-09-13 实施中发现，重要）

实施 B 时发现一处**同构缺口的修正**改变了整个事件周的水平：校准器此前把非五类的 `noise`
并入 `"other"`，而 env 一直把 noise 单列一类（`TYPE_ORDER_ALL`）。修正后（= env 的真实语义）
重估，并扫描保底条数 N（30 seed 代理，其余机制不变）：

| 保底 N | 0 | 2 | 3 | 4 | **5（当前配置）** | 6 |
|---|---|---|---|---|---|---|
| W13 悼念份额 | 0.310 | 0.431 | 0.462 | 0.455 | **0.515** | 0.581 |
| 与真实 0.414 之差 | −0.104 ❌ | **+0.018 ✅** | +0.048 ✅ | +0.041 ✅ | +0.101 ❌ | +0.167 ❌ |
| W13 悼念帖数 | 14.5 | 22.0 | 22.0 | 22.0 | 22.0 | 22.0 |
...
要点：① 规则本身仍必要（N=0 时低到 0.310，Δ−0.104 同样超判据）；② W13 悼念**帖数**在
N≥2 时恒为 22.0（= 悼念型 agent 总数，即该类型全员发言、其余类型被议程压到门槛下），
份额变化几乎全部来自**分母**——保底越多，非悼念类型的 `share_own` 被压得越低、
D<1 抑制越强（沉默螺旋被议程规则放大）；③ 排除 noise 本身效应量在 ±0.009 以内且符号不定
（|Δ| ≤ 0.01，远小于 seed 间噪声），是**口径清理**而非标定杠杆——此前认为它抬升份额的判断
作废；④ 此前"排除 noise 前后 0.431 vs 0.431"的对照同样**作废**——当时校准器尚未保留原始
类型，补丁里的 `!= "noise"` 从未触发。
...
- **口径区分（用户 2026-09-10 确认）**：注入分层只依据**各周帖子（内容）类型占比**，
  ...
  base）相互独立、互不混用
- **口径定档（用户 2026-09-13 裁定：维持原口径，不改）**：群体构成维持
  **W12–W22 全窗 + 发帖人口径（单类型用户 33,816，占比 94.25%）**，
  即现行 19/22/23/15/21。同轮评估并**否决**的三个替代方案（对照数见
  `datasets/zhangxf_labeled/composition_caliber.json`）：
  (a) **W12 单周口径**——退化为 营销80/教育16/其他3/梗0/悼念0，梗与悼念 agent 归零
  使"差异化激活既有玩梗群体"无法检验；
  (b) **事件前 W05–W12 口径**——同源退化（营销82/教育13/其他4/梗1/悼念0），
  事件前梗每周仅 0–6 帖、悼念≈0，五类中有三类在数据上不存在，估不出群体结构；
  (c) **≥2 帖 complete-case 口径**——营销 42/教育 20/梗 18/悼念 10/其他 10，
  口径更严但改变了基准对照的含义，本轮不采用。
  结论：现有构成中**梗 18.7%→18.2% 对单帖截断稳健**，营销(22.8%→41.8%)与
  悼念(22.2%→10.0%)不稳健——该敏感性已知悉并接受，**不另设构成稳健性臂**。
  同期发现的 HYPOTHESIS.md「梗文化 37.9% 用户」为旧口径残留（现口径为 18.7%，
  内容侧 13.6%≈13.7% 无误），**留待用户确认后回填**。
