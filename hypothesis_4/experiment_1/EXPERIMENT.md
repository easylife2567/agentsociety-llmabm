...
R = exp(−λ·cum_own/20)                                   （注意力衰减：收益侧疲劳折减）
发言当且仅当 U ≥ activity
```

R 的尺度（2026-09-12 用户裁定：50 → 20，λ 等效 ×2.5，『让退潮显现』）：真实平台总量是**深 V**
（W13 11394 → W18 1703，6.7 倍衰减 → W22 5338），而模拟的周总产量几乎平（52→45→44）——
R 太弱导致事件后的疲劳退潮没有显现。增强 R 后：① 退潮显现（趋势幅度 2.2→4.3）；
② 锯齿（抽样噪声，绝对幅度不变）相对不显眼（信噪比 0.45→**0.82**）；③ 核心效标
combined 玩梗份额 W19–W22 的拟合误差 RMSE 0.132→**0.043**（代理 16 seed 均值；
×3 及以上会在 W22 过冲）。机制：R 增强 → 其他类型更快退潮 → 份额分母缩小 → 玩梗份额
被动抬升；玩梗型累计曝光起步晚，二波不受损。尺度定义于共享纯函数
`custom/envs/curation_mechanisms.py::fatigue_factor`（agent 与校准脚本同源）。
...
**营销 23 / 悼念 22 / 其他 21 / 玩梗 19 / 讨论教育 15**（100 人；用户 2026-09-13 更正）。
...
- **口径区分（用户 2026-09-10 确认）**：注入分层只依据**各周帖子（内容）类型占比**，
  与 agent 群体构成（19/22/23/15/21，**发帖人口径**，仅用于群体装配与沉默螺旋基线
  base）相互独立、互不混用
