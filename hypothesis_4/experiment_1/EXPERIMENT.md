...
新群体 19/22/23/15/21 + W13 保底 + 官方置顶 + 营销 λ=0.05）**：W13 悼念份额 0.431 vs 真实
0.414（Δ +0.017，判据 ±0.08 ✅）；玩梗 W19–W22 0.001/0.227/0.490/0.601 vs 真实
0.078/0.298/0.556/0.603（二波形状贴合）；五类构成平均 |差| 玩梗 0.030 / 悼念 0.068 /
教育 0.057 / 其他 0.060 / **营销 0.193（最大 0.285 @W19，已知偏差）**。尺度定义于共享纯函数
...
- **口径提示**：裁定口径是"哀悼**倾向分**前 5"，不是"5 条 mourning **类型**帖"。倾向分
  = 命中数/(字数/50)，短文本密度天然偏高，实测（s0）保底构成为
  mourning×3 + other×1（遗产/商业版图）+ **noise×1（乱码帖）**，s1 为 mourning×4 + marketing×1，
  s2 为 mourning×4 + noise×1。变体对照（30 seed 代理）：现状与"排除 noise 类型"几无差别
  （W13 悼念份额 0.431 vs 0.431），而"仅取 mourning 类型"会过冲（0.558，Δ+0.144 超判据）——
  **是否排除 noise/非 mourning 帖待用户裁定**
